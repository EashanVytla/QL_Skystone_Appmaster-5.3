...
public class QL_Auto_Skystone extends OpMode {
    ...
    static final double turnCone = 6.0;
    ...
    public void loop() {
        ...
        switch(mRobotState) {
            ...
            case STATE_BACK:
                ...
                break;
            case STATE_TURN2:
                if (Math.abs(getangle() - Math.PI / 2) < Math.toRadians(turnCone)) { //+ Math.toRadians(5.5)))
                    if (time.time() >= 0.7) {
                        drive.setPower(0.0, 0.0, 0.0);
                        memo = getForwardDist();
                    ...
                    time.reset();
                    drive.targetTurn(Math.PI / 2); //+ Math.toRadians(5.5)
                }
                drive.read(data);
                break;
            case STATE_CROSS:
